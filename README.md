...
macOS Terminal profile assets may later live under `assets/terminal/`. The expected manual import path is `assets/terminal/GitHub Dark.terminal`; the binary profile is not generated by Nix.
...
5. applies `nix#macos` with nix-darwin.
...
The configuration selects `GitHub Dark` as Terminal's default and startup profile. The profile itself must be imported manually. When available:

1. open `assets/terminal/GitHub Dark.terminal`;
2. allow Terminal to import it;
3. reapply the nix-darwin configuration if needed.
